...
{
    public partial class SpeedMeterSampleFour : ContentPage
    {
        ...
        private async Task<bool> getLayouts(AbsoluteLayout availableLayout)
        {
            ...
            try
            {
                ...
                {
                    NumbersLayoutFour numbersLayout = new NumbersLayoutFour(i);
        ...
        private async Task<bool> startRotationAnimation()
        {
            try
            {
                ...
                {
                    await AfterEffects(absUnits, currentUnitsChildNo);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void moveAbsTens(double interval)
        {
            try
            {
                ...
                if (stopAnimation)
                {
                    ...
                    currentTensChildNo = moveAbsCommon(absTens, currentTensChildNo, (interval/10));
                }
                else
                {
                    await AfterEffects(absTens, currentTensChildNo);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void moveAbsHundreds(double interval)
        {
            try
            {
                ...
                if (stopAnimation)
                {
                    ...
                    currentHundredsChildNo = moveAbsCommon(absHundreds, currentHundredsChildNo, (interval / 10));
                }
                else
                {
                    await AfterEffects(absHundreds, currentHundredsChildNo);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void moveAbsThousands(double interval)
        {
            try
            {
                if (stopAnimation)
                {
                    ...
                    currentThousandsChildNo = moveAbsCommon(absThousands, currentThousandsChildNo, (interval / 10));
                }
                else
                {
                    await AfterEffects(absThousands, currentThousandsChildNo);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> AfterEffects(AbsoluteLayout currentLayout, int currentItemNo)
        {
            try
            {
                double x, y, height, width;
                ...
                currentItemNo = ((absUnits.Children).IndexOf(_currentItem));
                System.Diagnostics.Debug.WriteLine(currentItemNo.ToString());

                if (currentItemNo == 0)
                {
                    _pastItem = absUnits.Children[(absUnits.Children.Count - 1)];
                    _currentItem = absUnits.Children[currentItemNo];
                    _nextItem = absUnits.Children[currentItemNo + 1];
                }
                else if (currentItemNo == (absUnits.Children.Count - 1))
                {
                    _pastItem = absUnits.Children[currentItemNo - 1];
                    _currentItem = absUnits.Children[currentItemNo];
                ...
                var _selectedItem = absUnits.Children[currentItemNo];
    ...
    public class NumbersLayoutFour : StackLayout
    {
        double screenHeight, screenWidth;
        public NumbersLayoutFour(int number)
        {
            screenHeight = (App.screenHeight * 1) / 100;
            screenWidth = (App.screenWidth * 1) / 100;
